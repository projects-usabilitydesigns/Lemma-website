...
import { agenciesFaqs } from "@/lib/solutions-agencies-page-data";
...
export function AgenciesFaq() {
  const [openId, setOpenId] = useState<string | null>(agenciesFaqs[0]?.id ?? null);
            ...
            return (
              ...
              >
                <button
                  type="button"
                  aria-expanded={open}
                  aria-controls={`agencies-faq-${item.id}`}
                  className={cn(
