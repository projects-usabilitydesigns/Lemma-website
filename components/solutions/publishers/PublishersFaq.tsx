...
import { publishersFaqs } from "@/lib/solutions-publishers-page-data";
...
export function PublishersFaq() {
  const [openId, setOpenId] = useState<string | null>(publishersFaqs[0]?.id ?? null);
            ...
            return (
              ...
              >
                <button
                  type="button"
                  aria-expanded={open}
                  aria-controls={`publishers-faq-${item.id}`}
                  className={cn(
