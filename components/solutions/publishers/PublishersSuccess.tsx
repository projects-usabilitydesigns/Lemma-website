...
import { publishersSuccess } from "@/lib/solutions-publishers-page-data";

export function PublishersSuccess() {
  return (
    <section
    ...
    >
      <Container>
        <FadeUp className="mx-auto mb-10 max-w-2xl space-y-4 text-center">
          <SectionLabel label={publishersSuccess.label} align="center" accent="blue" />
          <h2 className="font-heading text-[32px] font-semibold tracking-[-0.72px] text-[var(--color-ink)] md:text-[45px] md:leading-[50px]">
            {publishersSuccess.title}
          </h2>
          <p className="font-[family-name:var(--font-inter)] text-[18px] leading-[1.65] text-[var(--color-slate)] max-w-2xl mx-auto">
            {publishersSuccess.description}
          </p>
        </FadeUp>

        <FadeUp delay={0.08}>
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {publishersSuccess.stats.map((stat) => (
              <div
                key={stat.label}
