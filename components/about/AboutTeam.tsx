...
function TeamCard({ member, showRole = true }: { member: TeamMember; showRole?: boolean }) {
  return (
    <motion.article variants={staggerItem} className="group text-center">
      <div className="relative mx-auto mb-4 aspect-[3/4] w-full max-w-[280px] overflow-hidden rounded-[8px] bg-[#f0ebe6]">
        <Image
          src={member.image}
          alt={member.name}
          fill
      ...
      {member.linkedin ? (
        <Link
          ...
          className="mt-2 inline-flex size-8 items-center justify-center rounded-full text-[#0A66C2] transition-opacity hover:opacity-80"
        >
  ...
  return (
    <section id="team" className="bg-white pb-8 pt-8 md:pb-12 md:pt-10">
      <Container>
        ...
        <Stagger className="mx-auto grid max-w-6xl gap-x-16 gap-y-20 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:gap-x-20 lg:gap-y-24">
        ...
        <div className="mt-20 md:mt-24">
          ...
          <Stagger className="mx-auto grid max-w-6xl gap-x-16 gap-y-20 sm:grid-cols-2 lg:grid-cols-4 lg:gap-x-20 lg:gap-y-24">
            {advisors.map((member) => (
