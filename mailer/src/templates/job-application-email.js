...
function buildJobApplicationEmail(values) {
  ...
  const cvFilename = String(values.cvFilename ?? "").trim();
  const pageUrl = sanitizePageUrl(values.pageUrl);
  const subject = `Application for ${jobTitle}${jobId ? ` (${jobId})` : ""} - ${fullName}`;
...
<html lang="en">
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;">
            ...
            <tr>
              <td style="background:#09131a;padding:28px 32px;">
                <img src="cid:${JOB_LOGO_CID}" alt="Lemma" width="149" height="32" style="display:block;border:0;outline:none;height:32px;width:149px;" />
              </td>
            </tr>
            <tr>
              <td style="background:#008fdb;padding:28px 32px;">
                <p style="margin:0;color:#d7effa;font-size:11px;letter-spacing:1.6px;text-transform:uppercase;font-weight:700;">New job application</p>
                <h1 style="margin:8px 0 0;color:#ffffff;font-size:26px;line-height:32px;font-weight:700;">${escapeHtml(fullName)} applied${jobTitle ? ` for ${escapeHtml(jobTitle)}` : ""}</h1>
                <p style="margin:10px 0 0;color:#e8f6fd;font-size:15px;line-height:22px;">${escapeHtml(company)}${jobId ? ` · ${escapeHtml(jobId)}` : ""}</p>
              </td>
            </tr>
            <tr>
              <td style="background:#ffffff;padding:8px 32px 28px;border-left:1px solid #e6eaee;border-right:1px solid #e6eaee;">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                  <tr>
                    <td colspan="2" style="padding:20px 0 8px;color:#09131a;font-size:13px;font-weight:700;letter-spacing:0.4px;text-transform:uppercase;border-bottom:1px solid #eef2f4;">Applicant</td>
                  </tr>
                  ${row("Name", escapeHtml(fullName))}
                  ${row("Email", `<a href="mailto:${escapeHtml(email)}" style="color:#008fdb;text-decoration:none;">${escapeHtml(email)}</a>`)}
                  ...
                  <tr>
                    <td colspan="2" style="padding:20px 0 8px;color:#09131a;font-size:13px;font-weight:700;letter-spacing:0.4px;text-transform:uppercase;border-bottom:1px solid #eef2f4;">Role</td>
                  </tr>
                  ${row("Job title", displayValue(jobTitle))}
                  ${row("Job ID", displayValue(jobId))}
                  ${row("CV / Resume", cvFilename ? `<span style="font-weight:600;">${escapeHtml(cvFilename)}</span> (attached)` : "—")}
                ...
                <table role="presentation" cellpadding="0" cellspacing="0" style="margin-top:24px;">
                  <tr>
                    <td style="border-radius:999px;background:#008fdb;">
                      <a href="mailto:${escapeHtml(email)}?subject=${encodeURIComponent(`Re: Application for ${jobTitle}`)}" style="display:inline-block;padding:12px 22px;color:#ffffff;font-size:14px;font-weight:700;text-decoration:none;">Reply to ${escapeHtml(firstName || "them")}</a>
                    </td>
                  </tr>
                </table>
              </td>
  ...
  const text = [
    `New Lemma job application`,
    `${fullName}${jobTitle ? ` applied for ${jobTitle}` : ""}${jobId ? ` (${jobId})` : ""}`,
    "",
    `Name: ${fullName}`,
    `Email: ${email}`,
    `Phone: ${phone || "—"}`,
...
module.exports = { buildJobApplicationEmail };
