...
const DEMO_LOGO_CID = "lemma-logo";
...
function buildDemoRequestEmail(values) {
...
<html lang="en">
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;">
            ...
            <tr>
              <td style="background:#008fdb;padding:28px 32px;">
                ...
                <p style="margin:10px 0 0;color:#e8f6fd;font-size:15px;line-height:22px;">${escapeHtml(company)}${String(values.role ?? "").trim() ? ` · ${escapeHtml(String(values.role).trim())}` : ""}</p>
              </td>
            </tr>
            <tr>
  ...
  const text = [
    ...
    `Phone: ${phone || "—"}`,
    `Company: ${company}`,
    `Job title: ${String(values.jobTitle ?? "").trim() || "—"}`,
    `I am a: ${String(values.role ?? "").trim() || "—"}`,
...
module.exports = { buildDemoRequestEmail, DEMO_LOGO_CID };
